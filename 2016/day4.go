...
type ByNumAlpha []freqEntry

func (a ByNumAlpha) Len() int      { return len(a) }
func (a ByNumAlpha) Swap(i, j int) { a[i], a[j] = a[j], a[i] }
func (a ByNumAlpha) Less(i, j int) bool {
	if a[i].Num == a[j].Num {
...
func main() {
	input := strings.Split(`aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
outer:
	for _, row := range input {
		...
		for k, v := range top {
			...
			sort.Sort(ByNumAlpha(observed))
			observed = observed[0:5]
		}
		for i, c := range observed {
			if c.Num == 0 && i >= len(check) {
				continue
			}
			if rune(check[i]) != c.Char {
				continue outer
			}
		}
		sum += id
		...
		for _, c := range name {
			...
			fmt.Printf("%c", rune('a'+(int(c)-'a'+id)%26))
		}
		fmt.Println()
	}
	fmt.Println(sum)
